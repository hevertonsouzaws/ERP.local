...
<template>
    <div class="p-4 bg-gray-900 rounded-lg">
        ...
        <div v-else>
            <input 
                v-if="!clienteLogica.mostrarNovoCliente.value"
                type="text" 
                v-model="clienteLogica.clienteBusca.value" 
            ...
            <div v-if="clienteLogica.clientesFiltrados.value.length > 0 && clienteLogica.clienteBusca.value && !clienteLogica.mostrarNovoCliente.value"
                ...
                <div 
                    v-for="cliente in clienteLogica.clientesFiltrados.value" 
                    :key="cliente.uuid"
            ...
            <button 
                @click="clienteLogica.mostrarNovoCliente.value = !clienteLogica.mostrarNovoCliente.value"
            ...
            <div v-if="clienteLogica.mostrarNovoCliente.value" class="mt-3 p-3 bg-gray-600 rounded-lg">
                <input 
                    type="text" 
                    v-model="clienteLogica.novoClienteData.value.nome" 
                    placeholder="Nome Completo"
                    class="w-full p-2 bg-gray-700 border border-gray-500 rounded mb-2 text-sm"
                >
                <input 
                    type="text" 
                    v-model="clienteLogica.novoClienteData.value.telefone" 
                    placeholder="Telefone (Opcional)"
                    class="w-full p-2 bg-gray-700 border border-gray-500 rounded mb-3 text-sm"
                >
                <button 
                    @click="clienteLogica.cadastrarESelecionarCliente"
