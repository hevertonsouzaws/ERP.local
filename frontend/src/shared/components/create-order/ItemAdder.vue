...
<template>
  <div class="space-y-3 p-4 rounded-lg border border-gray-400">
    ...
    <div class="p-3 rounded-lg space-y-2 border border-gray-500 pb-4">
      ...
      <p v-if="itemLogica.pecaSelecionadaParaServico.value" class="text-sm text-white mt-2">
        ...
        <button
          ...
          class="text-xs text-red-400 ml-3"
        >
          Limpar seleção
        </button>
      </p>
    ...
    <div
      class="p-3 border border-gray-500 rounded-lg space-y-2"
    ...
    >
      ...
      <div class="flex space-x-2">
        ...
        >
          ...
          <option
            v-for="service in [...itemLogica.services.value].sort((a, b) =>
              a.name.localeCompare(b.name, 'pt-BR', {
                sensitivity: 'base',
              }),
            )"
    ...
    <div class="mt-3 max-h-48 overflow-y-auto space-y-2">
      ...
      >
        ...
        <ul class="ml-2 border-l border-gray-500 pl-3 space-y-1">
          ...
          >
            ...
            <div>
              <span class="text-xs font-semibold text-green-400"
                >R$ {{ (servico.quantidade * servico.unitPrice).toFixed(2) }}</span
              >

              <button
                @click="itemLogica.removerServico(item.uuid, servico.uuid)"
          ...
          <li v-if="item.servicos.length === 0" class="text-xs italic text-gray-200">
