...
<template>
    <div class="w-full border border-gray-500 py-3 p-6 rounded-xl shadow-xl text-white">
        ...
        <div class="flex mb-2">
            <div v-for="dia in diasDaSemana" :key="dia" class="w-[14.28%] text-center text-sm text-gray-200 font-semibold">
                {{ dia }}
            </div>
        </div>
        ...
        <div class="flex flex-wrap">
            ...
            <div v-for="dia in diasDoMes" :key="dia" class="w-[14.28%] h-[80px] flex justify-center items-center p-0.5">
                <button @click="selecionarDia(getDiaString(dia))" :class="[
                    'w-full h-full border border-gray-600 flex flex-col justify-center items-center rounded-xl text-white transition duration-150 relative',
